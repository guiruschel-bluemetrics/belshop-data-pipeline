...
--   AND IV.data          = V.data_venda
   AND CAST(IV.data AS DATE) = CAST(V.data_venda AS DATE)
WHERE CAST(V.data_venda AS DATE) BETWEEN TO_DATE('2021-01-01', 'yyyy-MM-dd') AND CURRENT_DATE
    --and V.data_venda >= DATE '2021-01-01'
  --AND V.data_venda <  DATE_ADD(CURRENT_DATE, 1)